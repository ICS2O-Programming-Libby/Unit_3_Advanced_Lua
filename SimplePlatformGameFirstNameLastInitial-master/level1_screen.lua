...
local spikes = audio.loadSound("Sounds/spikes.mp3")
local spikesSoundChannel
...
--TO ALL OF THESE FUNCTIONS I ADDED THE 3rd
...
local function onCollision( self, event )
    ...
    if ( event.phase == "began" ) then
        ...
        if  (event.target.myName == "spikes1") or 
            ...
            numLives = numLives - 1
            -- ADDED SOUND HERE 
            spikesSoundChannel = audio.play(spikes)
...
local function AddPhysicsBodies()
    ...
    physics.addBody(rightW, "static", {density=1, friction=0.3, bounce=0.2} )  -- ADDED THIS
...
local function RemovePhysicsBodies()
    ...
    physics.removeBody(rightW) -- ADDED THIS
...
-- ADDED THE LEFT ARROW CREATION 
    ...
    --[[rW = display.newLine( 0, 0, 0, display.contentHeight)
    rW.isVisible = true

    -- Insert objects into the scene group in order to ONLY be associated with this scene
    sceneGroup:insert( rW )]]
